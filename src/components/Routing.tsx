  ...
  useEffect(() => {
    ...
    const bikeShopMarkers =
      bikeShops && showPOIs.bikeShops
        ? bikeShops.elements
            .filter(
              (bikeShopFeature: OverpassFeature) => bikeShopFeature.lon && bikeShopFeature.lat,
            )
            .map((bikeShopFeature: OverpassFeature) => {
              const element = document.createElement("div");
              element.className = `rounded-[11px] min-w-[22px] text-center cursor-pointer border-1 bg-blue-300 text-white p-[3px]`;
              element.innerHTML =
              ...
              element.onclick = (e) => handlePOIClick(e, bikeShopFeature);

              const marker = new mapboxgl.Marker({ element })
                .setLngLat([bikeShopFeature.lon, bikeShopFeature.lat])
                .addTo(map);
